...
(function($) {
	...
	var attHandler = {	
		...
		addData : function(data) {  //动态生成数据列表
			...
			$.each(data, function(index, dataObj) {				
	    		...
	    		$('#'+$this.listBodyId).append($("<tr>"+chkTd+"<td>"+($this.cursor++)+"</td><td class='uploadify-list-title'><a href='javascript:void(0);' id='"+aEl+"'>"+dataObj.name+"</a></td><td >"+dataObj.createUser+"</td><td >"+dataObj.createTime+"</td></tr>"));
	    		$('#'+aEl).click(function() {
	...
	$.fn.UploadFile = function(settings){
		...
		$(this).after("<div class='upload-list'><table width='100%' border='0' cellpadding='0' cellspacing='0' class='upload-table'>"+
				"<thead><tr>"+this.checkTh+"<th width='40'>序号</th><th >附件名</th><th width='100'>创建人</th><th width='180'>创建时间</th></tr></thead>" +
